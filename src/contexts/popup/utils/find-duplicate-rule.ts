...
import { getCanonicalMatchValue } from '@/contexts/popup/utils/match-value';
import type { HeaderRule } from '@/types';

/**
 * matchType + 照合値 + headerName が一致する既存ルール（自分自身は除く）を探す。
 * 同じ対象・同じヘッダーへの重複設定を保存前に弾くための判定用。
 * 照合値は getCanonicalMatchValue で正規化してから比較する（raw値のままだと、
 * 末尾スラッシュの有無やpunycode/unicode表記の違いだけで実質同じ対象を
 * 見逃してしまうため）。
 */
